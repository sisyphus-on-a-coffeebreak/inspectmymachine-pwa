...
class VehicleEntryPassController extends Controller
{
    ...
    /**
     * Create a new vehicle entry pass
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $qrData = $this->qrCodeService->generatePayload($passId, 'vehicle_entry');

            // Create vehicle entry pass
            DB::table('vehicle_entry_passes')->insert([
                'id' => $passId,
            ...
            Log::info('Vehicle entry pass created', [
                'pass_id' => $passId,
                'vehicle_id' => $request->input('vehicle_id'),
                'created_by' => $user->id
            ]);
            ...
            return response()->json([
                ...
                'pass' => [
                    'id' => $passId,
                    'pass_number' => 'VE' . strtoupper(substr($passId, 0, 8)),
                    'vehicle_id' => $request->input('vehicle_id'),
                    'access_code' => $accessCode,
                    'qr_payload' => $qrData['qr_payload']
                ]
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to create vehicle entry pass',
                'error' => $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * List vehicle entry passes
     * GET /api/vehicle-entry-passes
     */
    public function index(Request $request): JsonResponse
    {
        try {
            $query = DB::table('vehicle_entry_passes')
            ...
            $passes = $query->get()->map(function ($pass) {
                return [
                    'id' => $pass->id,
                    'pass_number' => 'VE' . strtoupper(substr($pass->id, 0, 8)),
            ...
            Log::error('Error fetching vehicle entry passes: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Failed to fetch vehicle entry passes',
                'error' => $e->getMessage()
            ], 500);
        }
    }
