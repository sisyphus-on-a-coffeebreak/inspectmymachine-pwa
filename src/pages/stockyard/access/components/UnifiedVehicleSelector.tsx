...
export const UnifiedVehicleSelector: React.FC<UnifiedVehicleSelectorProps> = ({
  ...
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      ...
      // If filtering for vehicles in yard, first fetch gate passes to get vehicle IDs
      let vehiclesInYardIds: string[] = [];
      if (statusFilter === 'in_yard') {
        try {
          // Fetch gate passes with status 'inside' and type 'vehicle_inbound' (vehicles that entered and are still inside)
          const gatePassParams: any = {
            status: 'inside',
            type: 'vehicle_inbound',
            per_page: 1000, // Get all vehicles in yard
          };
          if (yardId) {
            gatePassParams.yard_id = yardId;
          }

          const gatePassResponse = await apiClient.get('/v2/gate-passes', { params: gatePassParams });
          const gatePasses = Array.isArray(gatePassResponse.data?.data)
            ? gatePassResponse.data.data
            : gatePassResponse.data?.data || [];

          // Extract vehicle IDs from passes that are inside
          vehiclesInYardIds = gatePasses
            .filter((pass: any) => pass.vehicle_id && pass.status === 'inside')
            .map((pass: any) => String(pass.vehicle_id));
        } catch (error) {
          // If gate pass fetch fails, fall back to empty array (no vehicles in yard)
          console.warn('Failed to fetch vehicles in yard:', error);
          vehiclesInYardIds = [];
        }
      }
      ...
      let vehicleData = Array.isArray(response.data)
        ? response.data
        : response.data?.data || [];

      // Filter to only vehicles in yard if statusFilter is 'in_yard'
      if (statusFilter === 'in_yard' && vehiclesInYardIds.length > 0) {
        vehicleData = vehicleData.filter((v: Vehicle) => 
          vehiclesInYardIds.includes(String(v.id))
        );
      } else if (statusFilter === 'in_yard' && vehiclesInYardIds.length === 0) {
        // No vehicles in yard, return empty array
        vehicleData = [];
      }
    ...
    <div>
      ...
      {(!selectedVehicle || mode === 'multiple') && (
        <VehicleDropdown
          ...
          // No need for filterFn - vehicles are already filtered by gate passes when statusFilter === 'in_yard'
          filterFn={undefined}
        />
      )}
